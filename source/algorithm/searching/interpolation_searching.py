...
def interpolation_searching(_list, element):
    ...
    if len_of_list == 1:
        ...
        if pos == 0: 
            pos = 1
        if _list[pos] == element:
            return pos
        elif _list[pos] > element:
            return interpolation_searching(_list[:pos], element)
        elif _list[pos] < element:
            idx = interpolation_searching(_list[pos:], element)
            return idx + pos if idx else None
